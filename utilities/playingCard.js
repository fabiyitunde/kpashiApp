...
const isntOffCorner = (h, w, cr) => (cx, cy, r) =>
  (cx >= cr && cx <= w - cr) || (cy >= cr && cy <= h - cr - 2)
    ? true
    : Math.pow(
        Math.pow(w / 2 - cr - Math.abs(w / 2 - cx), 2) +
          Math.pow(h / 2 - cr - 1 - Math.abs(h / 2 - 1 - cy), 2),
        0.5
      ) +
        r <
      cr;
    ...
    .forEach((o, oi, arr) => {
      ...
      do {
        ...
        (!cornerCheck(cx, cy, r + 3) ||
          circles.filter(
            p =>
              Math.pow(p.cx - cx, 2) + Math.pow(p.cy - cy, 2) <
              Math.pow(p.r + r + 4, 2)
          ).length)
      );
...
export const SmallCardBack = ({ xOffset, yOffset, cardWidth, cardHeight }) =>
  !deckBacks[`${cardHeight}/${cardWidth}/${20}`] ? (
    generateCardBacks(cardHeight, cardWidth, 20) ||
    SmallCardBack({ xOffset, yOffset, cardWidth, cardHeight })
  ) : (
    <g transform={`translate(${xOffset}, ${yOffset})`}>
      <rect
        ...
        stroke="black"
      />
      {deckBacks[`${cardHeight}/${cardWidth}/${20}`].circles.map(
        ({ cx, cy, r, op }, ki) => (
          <circle
            key={ki}
            r={r}
...
const suitFills = ["#ddddee", "white", "#ffe6cc", "white"];
...
export const SmallCard = ({
  ...
  <g transform={`translate(${xOffset}, ${yOffset})`} onClick={onClick}>
    ...
    <text
      fontWeight="bold"
      transform={"translate(7, 40) " + (rank !== 10 ? "" : "scale(0.5, 1)")}
    ...
    <g transform={`translate(${21},${60}) rotate(180)`}>{suitTokens[suit]}</g>
  </g>
);
...
export const SmallHand = ({
  ...
  >
    {cards.map((card, i) =>
      !card.rank ? null : hidden || card.hidden ? (
        <SmallCardBack
          key={i}
          cardWidth={cardWidth}
...
export const SmallEjemplo = () => (
  <div>
    <SmallHand
      cards={[{ rank: 1 }, { rank: 2 }, { rank: 3 }]}
      hidden={true}
      style={defHandStyle}
    />
    <SmallHand
